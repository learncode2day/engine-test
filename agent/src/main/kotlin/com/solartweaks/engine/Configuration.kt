...
internal fun loadConfig(file: File) =
...
private val KProperty<*>.isSerialized
    get() = visibility == KVisibility.PUBLIC
            && !hasAnnotation<Transient>()
            && javaField != null

private inline fun <reified T : Any> serializedPropertiesOf() =
    T::class.declaredMemberProperties.filter(KProperty<*>::isSerialized)

private val KClass<*>.serializedProperties
...
data class Configuration(
    ...
    val customMods: List<String> = listOf(),
...
data class Modules(
    ...
    val showAllThemes: ShowAllThemes = ShowAllThemes(),
    val trollLogo: TrollLogo = TrollLogo(),
) {
    val modules
...
@ModuleInfo(
    "Window Title",
    "Allows you to set a custom title for the game's window (note: because of limitations this only works pre-1.14)"
)
data class WindowTitle(
    ...
    @OptionInfo("Show Advanced Version", "Appends commit hash and branch to window title")
    val showAdvancedVersion: Boolean = true,
    override val isEnabled: Boolean = true
) : Module
...
@ModuleInfo(
    ...
    "Allows you to use cracked/offline Minecraft accounts on Lunar Client"
)
data class AllowCrackedAccounts(
    ...
    @OptionInfo("Cracked Skin", "The name of the premium player whose skin you want to use")
    val crackedSkin: String = "Steve",
    override val isEnabled: Boolean = false
) : Module
...
@ModuleInfo(
    "FPS Spoof",
    "Spoofs your FPS with a multiplier (note: this purely visual, eg. fake)"
)
data class FPSSpoof(
    @OptionInfo(
        "Multiplier",
...
@ModuleInfo(
    "Show All Themes",
    "Unlocks all of Lunar's seasonal themes!"
)
data class ShowAllThemes(override val isEnabled: Boolean = true) : Module

@Serializable
@ModuleInfo(
    "Troll Logo",
    "Shows a weird easter egg that normally has a 1 in 1000000000 chance of happening"
)
data class TrollLogo(override val isEnabled: Boolean = false) : Module
