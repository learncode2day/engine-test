...
internal fun shouldImplementItems() = minecraftVersion.id in listOf("v1_8", "v1_7", "v1_12")
val shouldImplementItemsCached by lazy { shouldImplementItems() }

internal fun initCapeSystemTweaks() {
...
private fun MethodTransformContext.whenImplementItems(impl: MethodVisitor.() -> Unit) = methodEnter {
    val label = Label()
...
private val playerConfiguration = finders.optifineClass("PlayerConfiguration") {
    methods {
        // if this doesn't match, thats good
        named("renderPlayerItems") { arguments count 4 }
...
private val playerConfigAccess by accessor<_, PlayerConfiguration.Static>(playerConfiguration) {
...
private val fileDownloadThread = finders.optifineClass("FileDownloadThread") {
    methods { "construct" { method.isConstructor() } }
}

private val fdtAccess by accessor<_, FileDownloadThread.Static>(fileDownloadThread) {
...
private val playerConfigReceiver = finders.optifineClass("PlayerConfigurationReceiver") {
    methods {
        "construct" { method.isConstructor() }
...
private val playerConfigRecvAccess by accessor<_, PlayerConfigurationReceiver.Static>(playerConfigReceiver) {
...
object ConfigRegistry {
    ...
    internal val cosmeticsServer = getModule<CapeSystem>().serverURL.replace("https://", "http://")
...
fun renderItems(model: Any?, player: Any?, playerName: String, x: Float, y: Float) = with(GlStateManager) {
    color(1f, 1f, 1f, 1f)
...
private val capeUtils = finders.optifineClass("CapeUtils") {
    methods { named("reloadCape") }
}

private val capeUtilsAccess by accessor<_, CapeUtils.Static>(capeUtils)
