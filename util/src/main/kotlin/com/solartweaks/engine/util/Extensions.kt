 ...
 */
fun ClassNode.methodByInvoke(insn: MethodInsnNode) = methods.find { it.name == insn.name && it.desc == insn.desc }
 ...
 */
fun asmTypeOf(internalName: String): Type = Type.getObjectType(internalName)
...
inline fun <reified T : AbstractInsnNode> AbstractInsnNode.nextOrNull(block: (T) -> Boolean): T? {
    var insn = next
...
inline fun <reified T : AbstractInsnNode> AbstractInsnNode.next(block: (T) -> Boolean): T = nextOrNull<T>(block)
    ?: error("No matching insn was found!")

inline fun <reified T : AbstractInsnNode> AbstractInsnNode.previousOrNull(block: (T) -> Boolean): T? {
    var insn = previous
...
inline fun <reified T : AbstractInsnNode> AbstractInsnNode.previous(block: (T) -> Boolean): T = previousOrNull<T>(block)
    ?: error("No matching insn was found!")
